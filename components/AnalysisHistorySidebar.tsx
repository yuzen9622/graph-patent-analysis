...
export default function AnalysisHistorySidebar({ collapsed, onToggle }: Props) {
	...
	async function handleDelete(id: string, e: React.MouseEvent) {
		...
		const res = await fetch(`/api/analyses/${id}`, { method: "DELETE" }).catch(
			() => null,
		);
		if (!res?.ok) {
			...
			>
				{!collapsed && (
					<div className="flex items-center gap-2 min-w-0">
						<History size={15} className="text-accent shrink-0" aria-hidden />
				...
				<Button
					...
					className="shrink-0 text-foreground hover:bg-black/5 dark:hover:bg-white/8 cursor-pointer"
				>
									...
									>
										<div className="flex items-start gap-2 min-w-0">
											...
											<Button
												...
												className="opacity-0 group-hover:opacity-100 shrink-0 size-4 rounded-md text-muted-foreground hover:text-error cursor-pointer"
											>
												<Trash2 className="size-[10px]" aria-hidden />
											</Button>
										</div>
