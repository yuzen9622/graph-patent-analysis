...
export default function AnalysisHistorySidebar({ collapsed, onToggle }: Props) {
  ...
  return (
    <aside
      className={[
        "flex flex-col h-full bg-[#020617]/60 backdrop-blur-xl border-r border-white/[0.07] shrink-0",
    ...
    >
      ...
      >
        {!collapsed && (
          <div className="flex items-center gap-2 min-w-0">
            <History
              ...
              aria-hidden
            />
            <span className="text-sm font-semibold primary-foreground">
              分析歷史
            </span>
            {entries.length > 0 && (
              <span className="ml-1 text-[0.65rem] primary-foreground bg-white/8 rounded-full px-1.5 py-0.5 font-mono leading-none">
                {entries.length}
              </span>
            )}
          </div>
        ...
        <button
          ...
          className="w-8 h-8 flex items-center justify-center rounded-lg primary-foreground hover:primary-foreground hover:bg-white/8 transition-colors duration-150 cursor-pointer shrink-0"
        >
        ...
        <div className="flex-1 overflow-y-auto">
          {entries.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-40 gap-3 px-4 text-center">
              <Clock size={28} className="text-background" aria-hidden />
              <p className="text-xs text-border leading-relaxed">
                完成分析後
                <br />
                記錄將顯示於此
              </p>
                ...
                <li key={entry.id}>
                  ...
                  >
                    <div className="flex items-start gap-2 min-w-0">
                      ...
                      <span className="text-xs primary-foreground leading-snug truncate flex-1 font-medium">
                        {entry.filename.replace(/\.xlsx$/i, "")}
                      </span>
                      <button
                        onClick={(e) => handleDelete(entry.id, e)}
                        aria-label={`刪除 ${entry.filename} 記錄`}
                        className="opacity-0 group-hover:opacity-100 shrink-0 w-4 h-4 flex items-center justify-center rounded-md primary-foreground hover:text-[#EF4444] transition-all duration-150 cursor-pointer"
                      >
                        <Trash2 size={10} aria-hidden />
                      </button>
                    </div>
                    <div className="pl-5 flex items-center gap-2 flex-wrap">
                      <span className="font-mono text-[0.6rem] text-border">
                        {entry.id.slice(0, 8)}
                      </span>
                      {entry.patentCount !== undefined && (
                        <span className="text-[0.6rem] text-border">
                          · {entry.patentCount} 筆
                        </span>
                      )}
                      <span className="text-[0.6rem] primary-foreground ml-auto">
