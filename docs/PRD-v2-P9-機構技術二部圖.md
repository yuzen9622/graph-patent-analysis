...
>
> 1. **②承諾的問題，現有視圖已可回答**：「哪個技術競爭者最多」＝概念視圖切「家」單位，節點大小已是機構家數（`selectConceptView`，unit=applicant 分支）；「哪些公司學校都在做同一技術」＝機構網絡圖的邊已帶 `shared_concepts` 清單（`selectInstitutionView`）。
...
>   1. `minSupport` 是 `GraphLayout` 的單一共用 state、`selectMode` 不重設 → 在概念視圖設「≥8 家」切到機構網絡會靜默變成「≥8 個共享概念」並清空畫面。
...
- 完全 join 上限＝機構數 × 概念數（開發樣本 56 × 186 ≈ 10,416）。**實際邊數＝Σ_概念 持有者數，目前未量測**，須實作後量測並回填本節。
- 降載機制：
