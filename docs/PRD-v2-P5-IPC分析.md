...
> **2026-08-09 修訂（脈絡圖補 UI）**：IPC 篩選的資料層本來就三視圖共用（S3），但脈絡圖
> Sidebar 只顯示年份與節點層、看不到 IPC 操作 UI。本次把「IPC 層級 slider ＋ 樹狀多選」
> 區塊（S8）抽取為共用元件，脈絡圖模式也渲染（`hasIpcData` 閘門不變）。
> 另：脈絡圖新增「兩檔共享概念」角標（僅 A／僅 B／共有／聯集／Jaccard，隨子集重算）與
> 最低支持度 slider（概念至少出現在 N 篇可見專利才保留；`selectContextView` 依概念頻率過濾，
> 預設 1 不過濾，回歸安全）。詳見 `docs/implementation-plan-2026-08-09.md`。
