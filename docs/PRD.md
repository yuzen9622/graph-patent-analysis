...
從 Excel 原始資料 + LLM 萃取結果，同時建立兩種用途不同的研究視圖：

1. **技術概念網路**：只顯示技術概念；實證主邊為同一篇專利內的概念共現。
2. **專利脈絡圖**：顯示三層來源結構，用於回答概念來自哪篇專利、由誰申請。
...
另建立概念共現邊：`kind: "cooccurrence"`、`support_count`（共同出現的不同專利篇數）、`jaccard`、`source_patents`。LLM 語意邊標記為 `kind: "semantic"`，保留逐篇 `evidence`，不得與共現支持數混為同一指標。

每篇專利內的概念先去除空白、刪除空字串並以完全相同字串去重；目前不做同義詞或本體對齊。LLM 關係只有在起點與終點都存在於該篇專利的概念清單時才納入。

**F-06 社群偵測**

- 對技術概念的**共現網路**執行 Louvain 社群偵測，使用 TypeScript 套件 **`graphology` + `graphology-communities-louvain`**
- 邊權重固定為 `support_count`；LLM 語意邊不參與社群計算
- `resolution = 1`、`randomWalk = false`；無共現邊的概念形成可重現的單點社群
- 每個社群自動分配顏色
- 社群標籤 = 該社群度數最高的概念節點名稱
...
穩定化完成後關閉物理引擎（`physics.enabled: false`）。座標僅供排版，畫面像素距離不得解讀為語意相似度、因果強度或研究變項；介面與匯出圖均固定顯示此限制。
...
- 技術概念網路：共現實線寬度依 `support_count` 平方根縮放，可設定最低支持門檻；邊詳情顯示 support、Jaccard 與來源專利
- LLM 語意關係為固定寬度紫色虛線、預設關閉、`physics: false`，不以模型權重改變幾何距離
- 年份篩選同時作用於技術概念網路與專利脈絡圖：概念模式依選定專利子集重新推導節點、關係、指標與時間元資料（社群分區保留），脈絡圖同步重算申請人篇數、概念篇數、節點大小與統計，避免混用全樣本數值
...
- 以同一套 view selector 預先產生兩種研究視圖，連同目前篩選值與圖譜資料序列化為自包含 `.html` 檔案（資料與 vis-network 程式皆內嵌，不依賴 CDN）
- 可直接用瀏覽器開啟，不需要伺服器
- 預設開啟下載時所選模式，並可在離線檔內切換「技術概念網路／專利脈絡圖」
- 動態資料透過安全 JSON 序列化與 `textContent` 顯示，避免專利文字形成可執行 HTML
