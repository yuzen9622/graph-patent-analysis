...
export interface GraphNode {
	...
	color: string;
	size: number;
	/** Overrides the type-derived vis-network shape (A/B diff view uses it). */
	shape?: string;
}
...
export interface GraphEdge {
	...
	/** Derived metrics on a view must all originate from this same scope (I1). */
	scope_id?: string;
	/** Overrides the kind-derived stroke colour (A/B diff view uses it). */
	color?: string;
	/** Overrides the kind-derived dash pattern; `false` forces a solid line. */
	dashes?: [number, number] | false;
}
...
export interface GraphData {
	...
	scope_id?: string;
	warnings?: {
		temporal_direction_conflict?: Array<{
			edge_id: string;
			forward: number;
			reverse: number;
		}>;
