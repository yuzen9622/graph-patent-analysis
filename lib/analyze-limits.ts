...
export function proxyBodyBytes(
	env: Record<string, string | undefined> = process.env,
): number {
...
export function validateUploadFiles(
	...
	for (const file of files) {
		if (!/\.xlsx$/i.test(file.name)) {
			return {
				status: 400,
				error: `不支援的格式「${file.name}」，請上傳 .xlsx 檔案。`,
			};
		}
		if (file.size === 0) {
...
export interface AnalyzeRequest {
	...
	patents: PatentRow[];
	/** Per uploaded source file, clamped to `[1, min(analyzeMaxPatents, patents.length * files)]`. */
	sampleSize: number;
	uploadIds: string[];
	filenames: string[];
	citations: Citation[];
...
/**
 * Clamps the per-source-file `sample_size` into `[1, min(max, patentCount * fileCount)]` (§5.2).
 *
 * The ceiling scales with the uploaded file count because the value is a
 * per-file cap: an n-file request may legitimately select up to n times more
 * rows than a single file holds.
 *
...
export function resolveSampleSize(
	...
	const files = Number.isFinite(fileCount)
		? Math.max(1, Math.floor(fileCount))
		: 1;
	...
	if (
		...
		return Math.max(1, Math.min(max, patentCount));
	}
	return Math.max(
		1,
		Math.min(
			Math.max(1, Math.min(max, patentCount * files)),
			Math.floor(value),
		),
	);
}
...
export function defaultSampleSize(
	dedupedCount: number,
	max: number = DEFAULT_LIMITS.analyzeMaxPatents,
): number {
...
export function validateAnalyzeBody(
	...
	if (
		...
		!ANALYZE_PROVIDERS.includes(provider as AnalyzeRequest["provider"])
	) {
		return {
			ok: false,
			failure: {
				status: 400,
				error: `provider must be one of: ${ANALYZE_PROVIDERS.join(", ")}`,
			},
		};
	}
	...
	if (!Array.isArray(patents) || patents.length === 0) {
		return {
			ok: false,
			failure: { status: 400, error: "patents must be a non-empty array" },
		};
	}
	...
	return {
		...
		value: {
			...
			sampleSize: resolveSampleSize(
				...
				(filenames.length > 0 ? filenames : legacyName).length,
			),
 ...
 */
export function toPatentExtras(
	patent: PatentRow,
