...
function conceptLine(node: GraphNode, unit: TooltipUnit): string {
  ...
  if (node.applicant_count === undefined) return `概念：${node.label}（${patents}）`
  const applicants = `${node.applicant_count} 家機構`
  const counts = unit === 'applicant' ? `${applicants}／${patents}` : `${patents}／${applicants}`
...
export function conceptTimeLines(node: GraphNode): string[] {
  const lines: string[] = []
  if (node.first_year !== undefined) lines.push(`首次出現：${node.first_year} 年`)
  if (node.median_year !== undefined) lines.push(`中位年：${node.median_year} 年`)
  if (node.last_year !== undefined) lines.push(`最近出現：${node.last_year} 年`)
  return lines
}
