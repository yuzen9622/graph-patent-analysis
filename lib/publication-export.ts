...
function interpolateByWidth(
  ...
  const t =
    (widthMm - WIDTH_ANCHOR_MIN) / (WIDTH_ANCHOR_MAX - WIDTH_ANCHOR_MIN);
...
export function primaryLabelCap(widthMm: number): number {
  return Math.max(
    1,
    Math.round(interpolateByWidth(widthMm, LABEL_CAP_AT_MIN, LABEL_CAP_AT_MAX)),
  );
}
 ...
 */
export function isFullLabelBlocked(
  nodeCount: number,
  widthMm: number,
): boolean {
  const threshold = interpolateByWidth(
    widthMm,
    FULL_LABEL_THRESHOLD_AT_MIN,
    FULL_LABEL_THRESHOLD_AT_MAX,
  );
...
export function computeDegrees(
  ...
  for (const node of nodes)
    info.set(node.id, { degree: 0, inDegree: 0, outDegree: 0 });
...
export function selectPrimaryLabels(
  ...
  for (const communityId of communityIds) {
    ...
    const rep = [...members].sort(
      (a, b) =>
        compositeScore(b) - compositeScore(a) || a.id.localeCompare(b.id),
    )[0];
