...
export const SOURCE_OVERLAP_COLOR = "#dc2626"; // 跨多個來源檔出現的共有概念（紅色）
...
function usesApplicantCommunityColour(options: GraphViewOptions): boolean {
  ...
  const isDefaultColour =
    options.colorMode === undefined || options.colorMode === "community";
...
function capabilityWarning(
  ...
  if (graph.methodology.cooccurrence_data === "unavailable") {
    ...
    warnings.push(
      "共現統計由舊資料保存的專利—概念成員關係重建；正式分析建議使用新版重新產生。",
    );
  }
  if (graph.methodology.semantic_provenance === "partial") {
    warnings.push(
      "舊資料只保留部分 LLM 關係來源；目前顯示的是可觀測來源，不代表完整支持篇數。",
    );
  }
...
export function applicantAvailability(graph: GraphData): ApplicantAvailability {
  const stored =
    graph.nodes.some(
      (node) =>
        node.type === "concept" && typeof node.applicant_count === "number",
    ) ||
    graph.edges.some(
      (edge) =>
        edge.kind === "cooccurrence" &&
        typeof edge.support_applicants === "number",
    );
...
function legacyCycleInfo(edges: GraphEdge[], nodes: GraphNode[]) {
  const medianById = new Map(
    nodes.map((node) => [node.id, node.median_year] as const),
  );
  return breakTemporalCycles(
    edges
      ...
      .map((edge) => ({
        ...
        delta_median:
          (medianById.get(edge.to) ?? 0) - (medianById.get(edge.from) ?? 0),
      })),
  );
}
...
function legacyIndeterminateCount(nodes: GraphNode[]): number {
  return nodes.filter(
    (node) => node.type === "concept" && node.temporal_legacy_unverified,
  ).length;
}
...
function temporalWarnings(
  ...
  if (!cycleWarnings?.length && indeterminate === 0 && !conflicts?.length)
    return undefined;
  return {
    ...(cycleWarnings?.length
      ? { temporal_cycles_broken: [...cycleWarnings] }
      : {}),
    ...(indeterminate > 0
      ? { legacy_temporal_indeterminate: indeterminate }
      : {}),
    ...(conflicts?.length ? { temporal_direction_conflict: conflicts } : {}),
  };
}

function selectConceptView(
  graph: GraphData,
  ...
  const cooccurrence = graph.edges.filter(
    (edge) =>
      edge.kind === "cooccurrence" && supportOf(edge) >= options.minSupport,
  );
  ...
  if (options.colorMode === "first_year") {
    ...
    nodes = applyCommunityApplicantsColour(
      nodes,
      graph.communities_applicants ?? [],
    );
  ...
  const medianById = new Map(
    nodes.map(
      (node) =>
        [
          node.id,
          node.temporal_legacy_unverified ? undefined : node.median_year,
        ] as const,
    ),
  );
  const scopedEdges = edges.map((edge) => {
    ...
    if (orientation?.from === "b")
      [projected.from, projected.to] = [projected.to, projected.from];
    return projected;
  });
  ...
  const citationProjection = graph.patent_citations
    ? projectCitationEvidence(
        ...
        allRaw,
        nodes,
        scopedEdges.filter((edge) => edge.kind === "cooccurrence"),
        scopeId,
      )
    : undefined;
  if (citationProjection) {
    for (const edge of scopedEdges) {
      ...
      const evidence = citationProjection.byPair.get(
        `${edge.from}\u0000${edge.to}`,
      );
  ...
  return {
    ...
    citationEdges: options.showCitations
      ? (citationProjection?.citationEdges ??
...
function applyCommunityApplicantsColour(
  ...
  return nodes.map((node) => {
    if (node.type !== "concept" || node.community_id_applicants === undefined)
      return node;
...
function selectedRawIds(
  ...
  const isFullRange =
    yearStart === graph.stats.year_range[0] &&
    yearEnd === graph.stats.year_range[1];
  if (
    (!files || files.length === 0) &&
    (!ipcKeys || ipcKeys.length === 0) &&
    isFullRange
  )
    return null;
  ...
  for (const node of graph.nodes) {
    ...
    const okYear =
      isFullRange ||
      (typeof year === "number" && year >= yearStart && year <= yearEnd);
...
function selectConceptViewFiltered(
  ...
  let nodes: GraphNode[] = graph.nodes
    ...
    .map((node) => {
      ...
      return {
        ...node,
        source_patents: Array.from(patents),
        frequency: patents.size,
        applicant_count: apps.size,
        size: conceptSize(count),
      };
    })
  ...
  if (options.temporalReference !== "full") {
    const yearByRaw = new Map(
      graph.nodes
        .filter(
          (node) => node.type === "patent" && typeof node.year === "number",
        )
    ...
    nodes = nodes.map((node) => {
      const years = (node.source_patents ?? [])
        ...
        .filter(
          (year): year is number => year !== undefined && isValidYear(year),
        );
  ...
  const conceptApplicantCount = new Map(
    Array.from(conceptApplicants.entries()).map(
      ([l, s]) => [l, s.size] as const,
    ),
  );
  ...
  const co = graph.edges
    ...
    .map((e) => {
      ...
      return inter > 0
        ? { id: e.id, from: e.from, to: e.to, support_count: inter }
        : undefined;
    })
  ...
  const edges: GraphEdge[] = co.map((e): GraphEdge => {
    const a = labelOf(e.from);
    const b = labelOf(e.to);
    const A = conceptPatents.get(a);
    const B = conceptPatents.get(b);
  ...
  const allEdges = [...cooEdges, ...semantic]
    ...
    .map((edge) => {
      ...
      const oriented = {
        ...edge,
        temporal_directed: orientation !== undefined,
        opacity: supportStrengthOpacity(edge.support_count ?? 0),
        scope_id: scopeId,
      };
      if (orientation?.from === "b")
        [oriented.from, oriented.to] = [oriented.to, oriented.from];
      return oriented;
    });
  ...
  for (const node of nodes) {
    ...
    communityNodeCounts.set(
      communityId,
      (communityNodeCounts.get(communityId) ?? 0) + 1,
    );
  }
  const communities = (
    useApplicantCommunity
...
function projectCitationEvidence(
  ...
  for (const citation of graph.patent_citations ?? []) {
    ...
    seen.add(link);
    for (const from of labelsByRaw.get(citation.from) ?? [])
      for (const to of labelsByRaw.get(citation.to) ?? []) {
        if (from === to) continue;
        const key = `${from}\u0000${to}`;
  ...
  const byPair = new Map<
    string,
    { supported: boolean; direction_conflict: boolean }
  >();
  ...
  for (const edge of relationEdges) {
    ...
    byPair.set(`${edge.from}\u0000${edge.to}`, {
      supported,
      direction_conflict,
    });
    if (direction_conflict)
      conflicts.push({ edge_id: edge.id, forward, reverse });
  }
  const medianByLabel = new Map(
    nodes
      .filter((node) => node.type === "concept")
      .map((node) => [node.label, node.median_year] as const),
  );
...
function selectContextView(
    ...
    .map((node): GraphNode => {
      if (node.type === "applicant") {
        ...
        return {
          ...
          size: applicantSize(patentCount),
        };
      }
      if (node.type === "concept") {
        const patents = Array.from(
          patentIdsByConcept.get(node.id) ?? [],
        ).sort();
        return {
          ...node,
  ...
  const activeNodes = nodes.filter(
    (node) =>
      node.type !== "concept" || (node.frequency ?? 0) >= options.minSupport,
  );
 ...
 */
function selectInstitutionView(
  graph: GraphData,
  options: GraphViewOptions,
): GraphViewData {
  const rawSel = selectedRawIds(graph, options);
  ...
  const activeEdges = edges.filter(
    (e) => (e.support_count ?? 0) >= options.minSupport,
  );
  ...
  for (const patentsByApp of appPatents.values())
    for (const p of patentsByApp) patents.add(p);
...
export function selectGraphView(
  ...
  if (options.mode === "institution")
    return selectInstitutionView(graph, options);
