...
export function buildGraph(
  ...
  for (const patent of patents) {
    ...
    const primaryApplicant = applicantNames[0] ?? '未知申請人'
    const primaryApplicantNode = getOrCreateApplicant(primaryApplicant)
    const patentColor = withOpacity70(primaryApplicantNode.color)
