  ...
  it("舊圖無家單位分區時，community_applicants 退為「篇」單位不破", () => {
    const legacy: GraphData = {
      ...
      nodes: graph.nodes.map((n) => ({
        ...n,
        community_id_applicants: undefined,
      })),
    };
    const view = selectGraphView(legacy, {
      mode: "concept",
      showSemantic: false,
      minSupport: 1,
      yearRange: [2020, 2021],
    ...
    expect(
      view.nodes.every((n) => n.color === "#111" || n.color === "#222"),
    ).toBe(true);
  });
});
describe("家單位門檻（Q3）", () => {
  ...
  it("unit=patent：門檻套 support_count；unit=applicant：門檻套 support_applicants", () => {
    const byPatent = selectGraphView(unitGraph, {
      mode: "concept",
      showSemantic: false,
      minSupport: 2,
      yearRange: [2020, 2021],
    ...
    const byApplicant = selectGraphView(unitGraph, {
      mode: "concept",
      showSemantic: false,
      minSupport: 2,
      yearRange: [2020, 2021],
  ...
  it("unit=applicant 節點大小改用家數", () => {
    const view = selectGraphView(unitGraph, {
      mode: "concept",
      showSemantic: false,
      minSupport: 1,
      yearRange: [2020, 2021],
...
describe("P2 來源檔（多檔比對）", () => {
  ...
  it("依來源著色：僅一檔＝該檔本色；跨檔＝共享紅色", () => {
    const view = selectGraphView(srcGraph, {
      mode: "concept",
      showSemantic: false,
      minSupport: 1,
      yearRange: [2020, 2021],
