...
describe("community analysis contract", () => {
  it("相同共現網路可重現，且改變 semantic overlay 不影響社群", () => {
    ...
    const first = detectCommunities(buildConceptNetwork(base));
    const repeated = detectCommunities(buildConceptNetwork(base));
    const overlayChanged = detectCommunities(buildConceptNetwork(withSemantic));
