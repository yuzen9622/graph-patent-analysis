...
describe("resource limits (§5.2)", () => {
	...
	it("falls back to the default when an override is missing or unusable", () => {
		...
		expect(readLimits({ [LIMIT_ENV_KEYS.uploadMaxFiles]: "nope" })).toEqual(
			DEFAULT_LIMITS,
		);
	});
});
...
describe("Next proxy's own body cap", () => {
	...
	it("follows the environment override so both caps can be lifted together", () => {
		expect(proxyBodyBytes({ [PROXY_BODY_ENV_KEY]: String(64 * MB) })).toBe(
			64 * MB,
		);
		expect(
			checkProxyBodyLimit(String(11 * MB), {
				[PROXY_BODY_ENV_KEY]: String(64 * MB),
			}),
		).toBeNull();
	});
});
...
describe("validateUploadFiles (§5.2, §5.3)", () => {
	...
	it("rejects a file over the single-file ceiling with 413, accepts one exactly on it", () => {
		...
		expect(validateUploadFiles([ok("big.xlsx", ceiling + 1)])?.status).toBe(
			413,
		);
	...
	it("rejects a batch whose summed bytes pass the total ceiling with 413", () => {
		...
		files.forEach((f) =>
			expect(f.size).toBeLessThan(DEFAULT_LIMITS.uploadMaxFileBytes),
		);
		expect(files.reduce((s, f) => s + f.size, 0)).toBeGreaterThan(
			DEFAULT_LIMITS.uploadMaxTotalBytes,
		);
		const failure = validateUploadFiles(files);
		expect(failure?.status).toBe(413);
	...
	it("honours overridden ceilings", () => {
		expect(
			validateUploadFiles(
				[ok("a.xlsx"), ok("b.xlsx")],
				limits({ uploadMaxFiles: 1 }),
			)?.status,
		).toBe(413);
		expect(
			validateUploadFiles(
				[ok("a.xlsx", 2048)],
				limits({ uploadMaxFileBytes: 1024 }),
			)?.status,
		).toBe(413);
	});
});
...
describe("sample size (§5.2)", () => {
	...
	it("keeps a per-file request instead of cutting it to the total patent count", () => {
		// 3 files x 100 per file, with only 120 rows actually selected.
		expect(resolveSampleSize(100, 120, 20000, 3)).toBe(100);
		expect(resolveSampleSize(100, 60, 20000, 3)).toBe(100);
	});

	it("still bounds a per-file request by the global analyze ceiling", () => {
		expect(resolveSampleSize(1e9, 30000, 20000, 3)).toBe(20000);
		expect(resolveSampleSize(1e9, 100, 20000, 3)).toBe(300);
	});
...
describe("validateAnalyzeBody (§5.2, §5.3)", () => {
	const patent = (id: string): PatentRow => ({
		id,
		title: `T${id}`,
		abstract: "A",
		applicant: "X",
	...
	it("rejects an unknown provider with 400", () => {
		...
		if (!result.ok) {
			...
			expect(result.failure.error).toBe(
				`provider must be one of: ${ANALYZE_PROVIDERS.join(", ")}`,
			);
		}
	});
	...
	it("accepts exactly the ceiling", () => {
		...
		const result = validateAnalyzeBody(
			{ provider: "gemini", patents },
			limits({ analyzeMaxPatents: 25 }),
		);
	...
	it("scales the sample_size ceiling with the uploaded file count", () => {
		const result = validateAnalyzeBody(
			body({ sample_size: 1e9, filenames: ["a.xlsx", "b.xlsx", "c.xlsx"] }),
		);
		expect(result.ok).toBe(true);
		if (result.ok) expect(result.value.sampleSize).toBe(6);
	});

	it("keeps a per-file sample_size larger than the selected row count", () => {
		// 3 undersized files: 100 per file was requested, only 60 rows exist.
		const result = validateAnalyzeBody({
			provider: "gemini",
			patents: Array.from({ length: 60 }, (_, i) => patent(String(i))),
			sample_size: 100,
			filenames: ["a.xlsx", "b.xlsx", "c.xlsx"],
		});
		expect(result.ok).toBe(true);
		if (result.ok) expect(result.value.sampleSize).toBe(100);
	});
	...
	it("reads the multi-file fields and keeps the legacy single ones working", () => {
		...
		const legacy = validateAnalyzeBody(
			body({ filename: "old.xlsx", upload_id: "u9" }),
		);
	...
	it("forwards the browser-side parser output (§5-5)", () => {
		const warnings = {
			...emptyWarnings(),
			ipc_unparseable: [{ patent: "p", column: "IPC5-1", value: "??" }],
		};
		const result = validateAnalyzeBody(
...
describe("toPatentExtras (§6.2)", () => {
	...
	it('omits absent values so saveGraph() writes NULL, never a 0/""/{} impostor', () => {
		...
		expect(
			toPatentExtras({ ...row, ipc5_raw: [], external_references: [] }),
		).toEqual({});
		expect(
			toPatentExtras({ ...row, patent_number: "", design_class: "   " }),
		).toEqual({});
		expect(toPatentExtras(row, "")).toEqual({});
	});
});
...
describe("multi-file panel against the real fixtures (§5.1, §5.3)", () => {
	it("per-file rows sum to the total, and the deduped count drives the sample default", () => {
		const parsed = parseExcelFiles([
			{
				buffer: buffer(path.join(FIXTURES, "format-b-sample.xlsx")),
				filename: "B.xlsx",
			},
			{
				buffer: buffer(path.join(FIXTURES, "format-a-sample.xlsx")),
				filename: "A.xlsx",
			},
		]);
...
describe("history display of a multi-file analysis (§6.3)", () => {
	it('labels one file plainly and several as "N 個檔" with every name', () => {
		...
		expect(formatUploadLabel(["a.xlsx", "b.xlsx"])).toBe(
			"2 個檔（a.xlsx、b.xlsx）",
		);
		expect(formatUploadLabel(["a.xlsx", "  ", ""])).toBe("a.xlsx");
	});
	...
	it("builds the sidebar entry from analysis_uploads", () => {
		...
		expect(entry.files.map((f) => f.url)).toEqual([
			"/api/files/u1",
			"/api/files/u2",
		]);
