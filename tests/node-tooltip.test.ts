...
describe('nodeTooltipLines — 單位不得被誤讀（意圖・決策 9）', () => {
  it('概念在「家」單位下把家數放最前面，且兩種計量都標單位', () => {
    const [line] = nodeTooltipLines(concept({ applicant_count: 3 }), 'applicant')
  ...
  it('機構節點在機構網絡視圖顯示涉足概念數（該視圖的節點大小語意）', () => {
    const [line] = nodeTooltipLines(applicant({ concept_count: 5 }))
    expect(line).toBe('申請人：國泰金融控股股份有限公司（涉足 5 個技術概念／12 篇專利）')
  })
  ...
  it('概念時間統計三行都出現，缺值的欄位不佔行', () => {
    const lines = nodeTooltipLines(
      concept({ applicant_count: 3, first_year: 2010, median_year: 2015.5, last_year: 2021 }),
      'applicant',
    )
    expect(lines.slice(1)).toEqual(['首次出現：2010 年', '中位年：2015.5 年', '最近出現：2021 年'])
    expect(conceptTimeLines(concept({ first_year: 2010 }))).toEqual(['首次出現：2010 年'])
